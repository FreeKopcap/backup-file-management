...
```bash
./removeZeroFiles.sh [-y|--yes] [-n|--dry-run]
```

**Опции:**
- `-y`, `--yes`: удаление без подтверждений (список файлов все равно выводится)
- `-n`, `--dry-run`: сухой прогон - только показ списка файлов, которые были бы удалены (без фактического удаления)

**По умолчанию (без опций):**
- Спрашивает подтверждение для каждого файла перед удалением

Скрипт запросит:
- **Маску файлов**: шаблон для поиска (например, `log*`, `*.tmp`, `test?`). Если оставить пустым (нажать Enter), будут найдены все файлы нулевого размера
